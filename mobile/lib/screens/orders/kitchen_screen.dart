...
class _KitchenScreenState extends State<KitchenScreen> with WidgetsBindingObserver {
  ...
  Future<void> _advance(Order order) async {
    final next = order.kitchenStatus == 'new' ? 'preparing' : (order.kitchenStatus == 'preparing' ? 'ready' : 'served');
    try {
      await _service.updateKitchenStatus(order.id, next);
      await _load();
    } on ApiException catch (e) {
  ...
  Widget build(BuildContext context) {
    final newOrders = _orders.where((o) => o.kitchenStatus == 'new').toList();
    final preparing = _orders.where((o) => o.kitchenStatus == 'preparing').toList();
    final ready = _orders.where((o) => o.kitchenStatus == 'ready').toList();
    return Scaffold(
      appBar: AppBar(
  ...
  Widget _orderCard(Order order) {
    final isNew = order.kitchenStatus == 'new';
    final isPreparing = order.kitchenStatus == 'preparing';
    final isReady = order.kitchenStatus == 'ready';
    final canCancel = order.status == 'open' || order.status == 'preparing' || order.status == 'ready';
    return Card(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Text('#${order.orderNumber}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)), const SizedBox(width: 8), _statusChip(_statusText(order.kitchenStatus), _statusColor(order.kitchenStatus)), const Spacer(), Text(_channelLabel(order), style: const TextStyle(fontWeight: FontWeight.w700))]),
  ...
  String _channelLabel(Order order) {
    final channel = switch (order.orderType) {
      'dine_in' => order.tableName ?? 'Dine-In',
      'delivery' => 'Delivery',
      _ => 'Takeaway',
    };
    return order.isQrOrder ? 'QR • $channel' : channel;
  }
