...
const orderItemSchema = new mongoose.Schema(
  {
    ...
    // Ad-hoc Quick Item created directly from the POS. These do not touch inventory.
    quickItem: {
    ...
    // Quantity already sent to kitchen. This lets later KOTs contain only
    // newly-added quantities and prevents duplicate kitchen tickets.
    kotSentQuantity: { type: Number, default: 0, min: 0 },
  },
  { _id: false }
);
...
const kotSchema = new mongoose.Schema(
  {
    kotNumber: { type: Number, required: true, min: 1 },
    items: { type: [kotItemSchema], required: true, default: [] },
...
const orderSchema = new mongoose.Schema(
  {
    ...
    refundStatus: { type: String, enum: ['not_required', 'pending', 'refunded'], default: 'not_required', index: true },
    refundedAmount: { type: Number, default: 0, min: 0 },
    refundedAt: { type: Date },
    refundReference: { type: String, trim: true },
    // Kitchen lifecycle is intentionally separate from billing/order status.
    // A paid takeaway can still be NEW in the kitchen, while an unpaid dine-in
    // tab can already be PREPARING.
    kitchenStatus: { type: String, enum: ['new', 'preparing', 'ready', 'served'], default: 'new', index: true },
    ...
    deliveryStatus: { type: String, enum: ['new','preparing','ready','out_for_delivery','delivered','cancelled'], default: 'new', index: true },
    voidedAt: { type: Date },
