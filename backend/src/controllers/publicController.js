...
const createPublicOrder = asyncHandler(async (req, res) => {
  const { tableNumber, customerName, customerPhone, items, note, clientRequestId, paymentMethod } = req.body;
  ...
  // Create the first KOT immediately. Kitchen lifecycle is separate from
  // payment/order status, so the customer can place an order while staff
  // still controls preparation and billing.
  order.kots.push({
    kotNumber: 1,
  ...
  for (const item of order.items) item.kotSentQuantity = item.quantity;
