...
const listTables = asyncHandler(async (req, res) => {
  ...
  const openStats = await Order.aggregate([
    { $match: { status: { $in: ['open', 'preparing', 'ready'] }, table: { $ne: null } } },
    {
      $group: {
        _id: '$table',
        count: { $sum: 1 },
        ...
        latestOpenOrderAt: { $max: '$createdAt' },
      },
    },
  ]);
  const statsMap = new Map(openStats.map((c) => [c._id.toString(), c]));

  const data = tables.map((t) => {
    const stat = statsMap.get(t._id.toString());
    const openOrderCount = stat?.count || 0;
    return {
      ...t.toObject(),
      openOrderCount,
      openItemCount: stat?.openItemCount || 0,
      latestOpenOrderAt: stat?.latestOpenOrderAt || null,
...
// GET /api/tables/:id — table + all its currently active (unpaid) orders.
// Active means open, preparing or ready; these states must stay consistent
// with listTables so the table selector/detail never disagree.
...
// DELETE /api/tables/:id/orders — clears all unpaid/open orders on a table.
const clearTableOrders = asyncHandler(async (req, res) => {
  const table = await Table.findOne({ _id: req.params.id, active: true });
  if (!table) throw new ApiError(404, 'Table not found.');

  const result = await Order.updateMany(
    { table: table._id, status: { $in: ['open', 'preparing', 'ready'] } },
    {
      $set: {
        status: 'voided',
        ...
        voidReason: 'Cleared from table',
      },
    },
  );

  res.json({ success: true, message: `${result.modifiedCount} order(s) cleared.` });
});
...
module.exports = { listTables, getTable, createTable, updateTable, deleteTable, clearTableOrders, getTableQr };
